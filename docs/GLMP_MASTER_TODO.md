...
38. **Ask for Cursor — check whether the AUTO-STATUS cron is still running
    (2026-08-03).** The AUTO-STATUS block at the bottom of this file is dated
    `2026-07-05T22:30:00-04:00` — a month stale against everything in the curated
    section above it. That block is supposed to be regenerated by
    `build_master_todo.py` each post-ingest cycle (per the AM/PM chain documented
    in `SCOUT_ARCHITECTURE.md` and item 23's fix pointing the cron path at the git
    checkout). No SSH access this session to check directly. **Ask:** confirm on
    Jetson whether `build_master_todo.py` is still firing on schedule and actually
    overwriting the AUTO-STATUS section on publish — if it's silently stalled,
    that's the same findability-pattern gap item 21 exists to catch (present in the
    file vs. actually being refreshed), just for this doc's own self-reporting
    block rather than corpus content.
