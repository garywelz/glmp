...
> **Status, end of day 2026-08-15: all three bugs below are fixed, deployed live, and committed.** Bug #1 (Cursor, backend Cloud Build `494b2056`), bug #2 (Claude Code, frontend Cloud Build `2ceda8db`), bug #3 (Cursor, same frontend build as #2). All now in git history on `copernicus-web@e83c19dda`. Full resolution write-up in `GLMP_MASTER_TODO.md` item 55. The bug descriptions below are left as originally written (diagnosis, not narrated as history) since the exact root causes remain the reference for anyone touching this code again.
