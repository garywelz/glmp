...
IN = os.path.join(BASE, "q2_pmids_raw.json")
OUT = os.path.join(BASE, "q2_dedupe.json")
    ...
    for snap in db.get_all(refs):
        if snap.exists:
            ...
            existing[snap.id] = {
                ...
                "already_q2": "glmp-q2" in (data.get("question_scope_ids") or []),
                "run_id": data.get("run_id"),
            }
    print(f"checked {min(i+CHUNK, len(ids))}/{len(ids)}, found so far: {len(existing)}")

already_q2 = sum(1 for v in existing.values() if v["already_q2"])
print(f"\nTotal candidates: {len(ids)}")
print(f"Already in corpus: {len(existing)}")
print(f"Already tagged glmp-q2 (should be 0, pre-write): {already_q2}")
