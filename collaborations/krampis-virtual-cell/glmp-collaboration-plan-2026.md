...
- **CopernicusAI** is the literature corpus — the raw material from which sequence-logic ground truth is extracted at scale. At 100K+ papers, it becomes the largest curated corpus of regulatory biology literature in existence, systematically annotated for circuit topology and sequence content.
...
Five working papers form the existing foundation of the series. The table below is the definitive cross-reference: the short names used throughout this document, the full titles, the **proposed new GitHub filenames** (see File Renaming section below), and the GCS HTML preview URLs.

| Short name used in this document | Full title | GitHub filename (proposed) | GCS preview |
|---|---|---|---|
...
## File Renaming — Required Before Sending to Krampis

The current GitHub filenames in the repository do not correspond obviously to the paper titles shown in the GCS navigation or to the short names used in this document. The following renames should be made before sharing the repository with Krampis, so that filenames are immediately readable from both the GCS site and this plan.

Execute in Cursor terminal from the repository root:
...
git add -A
git commit -m "Rename working papers to match GCS navigation labels and plan document short names"
git push
```

After renaming, also add the two new files from this session:

```bash
# From repo root
cp /path/to/GLMP_Krampis_Plan.md collaborations/krampis-virtual-cell/glmp-collaboration-plan-2026.md
cp /path/to/README.md collaborations/krampis-virtual-cell/README.md

git add collaborations/krampis-virtual-cell/glmp-collaboration-plan-2026.md
git add collaborations/krampis-virtual-cell/README.md
git commit -m "Add collaboration plan and updated README"
git push
```
...
- **Biological review of the lac operon sequence annotation section** — validation that the *lacO* sequence, CRP binding site, and spatial context are correctly described and that the logical interpretation (NOT gate, CONDITIONAL) accurately reflects the molecular biology.
...
**Hardware:** The Jetson Nano (reComputer J1010, 128GB) handles pre-processing — PDF parsing, metadata extraction, embedding generation, and lightweight motif scanning (FIMO/MEME suite) — before cloud submission to GCP. This offloads routine computation from GCP and reduces cloud costs while enabling local-first ingestion.
...
### GLMP Flowcharts — toward thousands of charts

Expansion is organized in six batches, each contributing a coherent scientific deliverable to the grammar-decoding program:
...
| **Synthesis** — *Genomic Regulatory Complexity and the Limits of Perturbation Prediction* | On bioRxiv as working draft |
...
5. Begin Batch 1 flowchart expansion — synthetic biology ground-truth circuits
6. Begin `flowchart-source-papers.tsv` manifest — circuit names and initial DOIs for existing flowcharts
7. Script Firestore `glmp_relevant` backfill pass over existing 59K papers

**Krampis — begins immediately:**
8. Review lac operon sequence annotation section draft for biological accuracy (once Welz draft is ready)
9. Contribute to sequence annotation schema definition — field validation for human circuits beyond prokaryotes
10. Identify RegVelo configuration for K562 GRN
11. Review and extend `flowchart-source-papers.tsv` manifest — validate DOIs and fill gaps for hematopoietic and virtual cell circuits

**Joint — first coordinated deliverable (~3–4 months):**
9. Methods paper revised draft → internal review → bioRxiv posting
10. RPE1 replication results → combined with RegVelo comparison → replication paper submission
