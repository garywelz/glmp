GLMP · Synthesis for biology · Working draft (not yet submitted)
...
**How to read this document.** Full mathematical development (foundational typology, dependency among axiomatic systems, proof-theoretic ordinals, and formal logic) appears in [Paper I](https://github.com/garywelz/glmp/blob/main/collaborations/krampis-virtual-cell/paper-I-foundational-typology.md). The mapping from Boolean and temporal operators to molecular mechanisms, transcriptome-as-state, and nine testable predictions appears in [Paper II](https://github.com/garywelz/glmp/blob/main/collaborations/krampis-virtual-cell/paper-II-genome-as-computer.md). Protocol detail, hypothesis register, robustness checks, and extended discussion of model scores appear in [Paper III](https://github.com/garywelz/glmp/blob/main/collaborations/krampis-virtual-cell/paper-III-empirical-sequel.md). Here we integrate the story for a biological audience and emphasize what can be tested next at the bench or in silico without reading the formal side in depth.
...
### 1.1 Two typing axes, kept distinct

A note on vocabulary, because the GLMP program now works along two distinct axes that share the word "class," and conflating them would confuse more than clarify.

This paper concerns the **feedback-topology axis**: circuits are sorted (Classes I–V) by the *dynamical structure* of their regulatory network — whether it is acyclic, closes a negative loop, supports bistability, oscillates, or rewrites itself. That axis is inferred from prior-knowledge network graphs, and it predicts, as shown below, how hard a circuit is for current models to forecast.

A second, separate axis — developed in the project's sequence-decoding work — types the **Boolean logic encoded in regulatory DNA**: whether a promoter's binding-site arrangement implements a NOT, an AND, or an OR. That axis is read from sequence rather than network topology, and it answers a different question — not *how the circuit behaves dynamically* but *what logic is written into its cis-regulatory DNA*.

The two are complementary views of the same object and are not interchangeable. Where earlier drafts used bare numerals for both, this synthesis reserves "Class I–V" strictly for the feedback-topology ladder and refers to the sequence-level categories by their logic-gate names. The section on sequence-level decoding below reports recent progress on that second axis, where a concrete definitional question has arisen that bears directly on how combinatorial logic should be typed.
...
One combinatorial primitive deserves explicit caution, because pinning it down turns out to be a biological question rather than a modeling one. A logical **AND** at a promoter should mean that two inputs are *jointly required* — the output is a non-separable function of both. But "both regulators present" is not the same as "both jointly required." The lac operon makes the gap concrete: sequence-level decoding now confidently locates both an activator (CRP) and a repressor (LacI) at the promoter, yet not as cooperatively spaced sites. Whether that configuration is a genuine combinatorial AND depends on cooperativity — joint necessity — rather than mere co-occurrence, and that is a molecular-biology judgment we are currently seeking rather than asserting. We flag it here because the choice propagates: how combinatorial AND is defined at one well-characterized promoter sets how it is detected and reported everywhere. Marking that boundary honestly is, in the GLMP idiom, itself a finding.
...
Readers who prefer diagrams to axioms may consult the GLMP methods primer on Mermaid flowcharts for perturbation design, which walks through lac-operon style logic charts, hybridization with RegulonDB-style entity completeness, and layered use of public databases without requiring any formal background ([HTML draft](https://github.com/garywelz/glmp/blob/main/collaborations/krampis-virtual-cell/mermaid-flowcharts-smarter-perturbation-design.md)).

## 6A. Recent progress: sequence-level logic decoding in *E. coli*

Parallel to the mammalian topology analysis, the project has been testing whether the *Boolean logic* of a circuit can be recovered directly from regulatory DNA in a system where the ground truth is unusually secure: *E. coli*. The current, deliberately bounded result is that the decoder is a **validated repression detector**. On reference-true promoter windows it reliably recovers NOT-gate logic — LacI, TrpR, and LexA repression — with non-circular validation and locked confidence thresholds. It does not yet type activation or combinatorial-AND logic, and we report that limit plainly rather than smooth it over.

Three findings from that work are worth carrying into the broader program, because each is a methodological caution as much as a result:

1. **Validation strategy scales with regulator breadth.** A regulator acting across many operons (e.g., CRP) admits held-out validation; a single-operon regulator (e.g., LacI on lac alone) does not, because there is no independent site to hold out. The appropriate validation design is not fixed — it depends on how broadly the regulator acts.

2. **Motif libraries must match the domain of life.** Applying a eukaryotic motif library (JASPAR CORE) to prokaryotic DNA fabricates apparent logic gates from noise. An early, tempting combinatorial call on lac rested on exactly this contamination; removing it downgraded the headline example to a simpler, honest classification. The lesson generalizes: cross-domain motif reuse is a silent source of false structure.

3. **Spacing alone cannot type single-activator combinatorial logic.** A crude cooperative-spacing heuristic cannot distinguish genuine combinatorial control from the mere co-presence of regulators — the definitional gap noted in §6.

Together these place the sequence axis on the same footing as the topology axis: useful, empirically grounded, and explicit about where it stops. They also motivate the open lac question, which a molecular-biology review is expected to resolve.
...
A further convergence has emerged since this synthesis was first drafted. Arc Institute's **Evo 2**, a DNA foundation model trained on 9 trillion base pairs across all three domains of life and published in *Nature* in 2026 (Brixi *et al.*), learns regulatory and functional structure directly from sequence at scale. Notably, its own mechanistic-interpretability analyses — using sparse autoencoders, the same tools now central to neural-network interpretability — show that Evo 2 *autonomously* learned to represent transcription-factor binding sites, exon–intron boundaries, and protein structural elements, with no supervision toward any of them. That transcription-factor binding sites emerge as learned features is directly germane here: they are precisely the alphabet on which GLMP's logic-typing operates. Complementary work from Goodfire provides interactive visualizations of those features. This points to a natural, complementary role for a framework like GLMP's — as an *interpretability lens* and a *validation target* for what such models have internalized about regulatory control. Where the topology analysis above asks how well virtual-cell models predict perturbation outcomes, the interpretability question is different: not only *whether* a foundation model is right, but *what regulatory logic it has learned*, checked against circuits whose logic we can type by hand. Anchoring learned representations against ground-truth circuits like lac is exactly the kind of validation that program needs, and it ties GLMP's typology directly to frontier genomic AI.
...
5.  Welz G. *Primitive Relations, Computational Complexity, and a Conjecture on the Genomic Computational Class* (GLMP Working Paper I, 2026). [HTML](https://github.com/garywelz/glmp/blob/main/collaborations/krampis-virtual-cell/paper-I-foundational-typology.md)
6.  Welz G. *The Genome as Computer: Logical Primitives, Runtime States, and the Computational Limits of Biological Prediction* (GLMP Working Paper II, 2026). [HTML](https://github.com/garywelz/glmp/blob/main/collaborations/krampis-virtual-cell/paper-II-genome-as-computer.md)
7.  Welz G. *Circuit Class Predicts Virtual Cell Model Accuracy: An Empirical Test of the Genomic Computational Complexity Hypothesis* (GLMP Working Paper III, 2026). [HTML](https://github.com/garywelz/glmp/blob/main/collaborations/krampis-virtual-cell/paper-III-empirical-sequel.md)
...
9.  Brixi G, Durrant MG, Ku J, *et al.*, Hsu PD, Hie BL. Genome modelling and design across all domains of life with Evo 2. *Nature* 652, 1349–1361 (2026). https://doi.org/10.1038/s41586-026-10176-5 (preprint: *bioRxiv*, 2025, https://doi.org/10.1101/2025.02.18.638918).
10. Gorton L, Wang N, Nguyen N, Deng M, Ho E, Balsam D, McGrath T. Interpreting Evo 2: Arc Institute's next-generation genomic foundation model. Goodfire (2025). https://www.goodfire.ai/research/interpreting-evo-2
...
Working draft — not yet submitted · July 2026  
Intended venue: systems / computational biology  
